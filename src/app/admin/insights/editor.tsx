...
export default function InsightsEditor({ params }: { params: Promise<{ id?: string }> }) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            let error;
            if (isEdit) {
                ({ error } = await supabase.from('insights').update(dataToSave).eq('id', id));
            } else {
                ({ error } = await supabase.from('insights').insert([dataToSave]));
            }

            if (error) throw error;

            setMessage({ type: 'success', text: isEdit ? "Insight updated successfully" : "Insight created successfully" });
        ...
        <div className="max-w-5xl mx-auto space-y-10">
            ...
            <div className="flex items-center justify-between">
                ...
                >
                    ...
                    <span>{isEdit ? "Update Insight" : "Publish Insight"}</span>
                </button>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        ...
                        <div className="space-y-8 md:col-span-2">
                            <div className="space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted flex items-center">
                                    Insight Title <span className="text-red-500 ml-1">*</span>
                                </label>
                                <input
                                    type="text"
                                    required
                                    placeholder="e.g., The Future of Legal Tech in Africa"
                        ...
                        <div className="space-y-3 md:col-span-2">
                            ...
                            <textarea
                                rows={15}
                                required
                                placeholder="Write the full insight analysis here..."
