...
export default function EventManagement() {
    ...
    async function handleAiGenerate() {
        if (!confirm("Trigger AI to generate a fresh event?")) return;
        setActionLoading(true);
        try {
        ...
        <div className="flex h-[calc(100vh-12rem)] bg-white border border-border rounded-sm overflow-hidden shadow-sm font-sans">
            ...
            <div className="w-1/3 border-r border-border flex flex-col bg-[#fcfcfc]">
                <div className="p-6 border-b border-border bg-white flex justify-between items-center">
                    <h2 className="font-serif text-xl font-black italic text-brand">Private Events</h2>
                ...
                <div className="flex-grow overflow-y-auto">
                    {loading ? (
                        ...
                        <div className="p-10 text-center text-muted italic text-xs">No events found.</div>
                    ) : events.map((item) => (
                        <button
                            key={item.id}
                        ...
                        >
                            ...
                            <div className="flex items-center space-x-2">
                                <span className="text-[8px] uppercase tracking-widest font-black bg-secondary/10 text-secondary px-2 py-0.5 rounded-sm">{item.event_type}</span>
                            </div>
                        </button>
                    ))}
                </div>
                ...
                {isEditing ? (
                    <div className="p-10 max-w-2xl mx-auto w-full space-y-8">
                        <div className="flex justify-between items-center">
                            <h2 className="font-serif text-3xl font-black italic">{selectedItem ? "Edit Event" : "New Event"}</h2>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    placeholder="Strategic Commercial Litigation"
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-muted">Event Type</label>
                                    <select
                                    ...
                                    >
                                        <option value="webinar">Exclusive Webinar</option>
                                        <option value="roundtable">Virtual Roundtable</option>
                                    </select>
                                </div>
                                <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-muted">Highlights (Comma separated)</label>
                                <input
                                    type="text"
                                    className="w-full p-3 border border-border outline-none focus:border-secondary transition-all text-sm"
                                    placeholder="Briefing 1, Briefing 2, Briefing 3"
                            ...
                            >
                                {actionLoading ? <Loader2 className="animate-spin mx-auto" size={16} /> : (selectedItem ? "Save Changes" : "Create Event")}
                            </button>
                        </form>
                    </div>
                ) : selectedItem ? (
                    <div className="p-16 space-y-12">
                        <div className="flex justify-between items-start">
                            <div className="space-y-4">
                                <div className="inline-flex items-center space-x-2 px-2 py-1 bg-secondary/10 border border-secondary/20 rounded-full">
                                    ...
                                    <span className="text-secondary font-black text-[8px] uppercase tracking-widest leading-none">{selectedItem.event_type}</span>
                                </div>
                                <h2 className="font-serif text-5xl font-black italic">{selectedItem.title}</h2>
                                <p className="text-xl text-muted font-light max-w-2xl">{selectedItem.description}</p>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-12">
                            <div className="space-y-8">
                                <h3 className="text-[10px] font-black uppercase tracking-widest text-muted italic border-b border-border pb-4">Event Highlights</h3>
                                <div className="space-y-4">
                                    {Array.isArray(selectedItem.highlights) && selectedItem.highlights.map((h: string, i: number) => (
                            ...
                            <div className="space-y-8">
                                <h3 className="text-[10px] font-black uppercase tracking-widest text-muted italic border-b border-border pb-4">Metadata</h3>
                                <div className="space-y-4">
                                    <div className="flex items-center text-xs text-black">
                                        <Calendar size={14} className="mr-3 text-secondary" /> Created: {new Date(selectedItem.created_at).toLocaleDateString()}
                                    </div>
                                    <div className="flex items-center text-xs text-black">
                                        <Clock size={14} className="mr-3 text-secondary" /> Last Updated: {new Date(selectedItem.updated_at || selectedItem.created_at).toLocaleDateString()}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="flex-grow flex flex-col items-center justify-center space-y-6 opacity-30">
                        ...
                        <p className="font-serif text-2xl italic">Select or create a private event</p>
                    </div>
                )}
            </div>
        </div>
